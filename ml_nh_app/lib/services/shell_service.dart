...
class ShellService {
  ...
  static const _channel = MethodChannel("com.nh_skin_ml/app_launcher");

  Future<bool> launchGame() async {
    logger.log("Launching Mobile Legends...");
    try {
      final bool success = await _channel.invokeMethod('launchGame');
      if (success) {
        logger.log("[+] Mobile Legends launched successfully!");
        return true;
      }
      return false;
    } on PlatformException catch (e) {
      logger.log("[-] Failed to launch Mobile Legends: ${e.message}");
      return false;
    }
  }
}

...
class TweaksService {
  ...
  Directory _findRealSourceDir(Directory startDir) {
    final list = startDir.listSync();
    if (list.length == 1 && list.first is Directory) {
      return _findRealSourceDir(list.first as Directory);
    }
    return startDir;
  }
  ...
  Future<bool> injectTweakZip(String name, String url, String zipName) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final resolvedDir = _findRealSourceDir(extractPath);
        logger.log("[+] Resolved source directory for copy: ${resolvedDir.path}");
        ...
        if (mode == "Root Mode") {
          ...
          final ok2 = await shellService.executeRoot("cp -r \"${resolvedDir.path}/.\" \"$remoteAssetsDir\"");
          ok = ok1 && ok2;
        } else if (mode == "Shizuku") {
          final ok1 = await shellService.executeShizuku("mkdir -p $remoteAssetsDir");
          final ok2 = await shellService.executeShizuku("cp -r \"${resolvedDir.path}/.\" \"$remoteAssetsDir\"");
          ok = ok1 && ok2;
        }
  ...
  Future<bool> uninstallTweakZip(String name, String url, String zipName) async {
    logger.log("Preparing removal of custom tweak: $name...");
    await shellService.requestStoragePermission();
    try {
      final zipFile = File("/sdcard/Download/$zipName");
      List<int> bytes;
      if (zipFile.existsSync()) {
        ...
        if (response.statusCode == 200) {
          ...
          logger.log("[-] Failed to download tweak metadata for removal.");
          return false;
        }
      }
      ...
      final entities = resolvedDir.listSync(recursive: true);
      for (var entity in entities) {
        if (entity is File) {
          final relPath = entity.path.replaceAll(resolvedDir.path + Platform.pathSeparator, "").replaceAll("\\", "/");
          filesToDelete.add(relPath);
        }
      }

      StringBuffer deleteCmds = StringBuffer();
      for (var relPath in filesToDelete) {
        deleteCmds.write("rm -f \"$remoteAssetsDir$relPath\"; ");
      }

      bool ok = false;
      if (mode == "Root Mode") {
        ok = await shellService.executeRoot(deleteCmds.toString());
      } else if (mode == "Shizuku") {
        ok = await shellService.executeShizuku(deleteCmds.toString());
      }

      if (ok) {
        logger.log("[+] Successfully reverted custom tweak: $name to default!");
        return true;
      } else {
        logger.log("[-] Failed to delete modded files.");
        return false;
      }
    } catch (e) {
      logger.log("[-] Tweak removal failed: $e");
      return false;
    } finally {
      try {
        final extractPath = Directory("/sdcard/Download/.tweak_extract");
  ...
  Future<bool> injectModZip(String zipName) async {
    ...
    try {
      ...
      for (final relPath in computedHashes.keys) {
